...
if FREQ_SWEEP == True:
    ...
    wds = np.linspace(wr - 3e-3 * 2 * np.pi, wr + 3e-3 * 2 * np.pi, 300, endpoint=False)
else:
    # only one frequency
...
for wd in wds:
    ...
    if HAMILTONIAN == 'normal':
        ...
        H_f = [H_coupling, [H_drive1_f, H_drive1_coeff], [H_drive2_f, H_drive2_coeff]]
    ...
    if NUMBER_TRAJECTORIES == 0:
        ...
        for n in range(NUMBER_TRAJECTORIES):
            ...
            R_eg_g.append(calc_R_new(m_g, temp_e, temp_g))
            R_eg_e.append(calc_R_new(m_e, temp_e, temp_g))
            R_eg_f.append(calc_R_new(m_f, temp_e, temp_g))

            R_fe_g.append(calc_R_new(m_g, temp_f, temp_e))
            R_fe_e.append(calc_R_new(m_e, temp_f, temp_e))
            R_fe_f.append(calc_R_new(m_f, temp_f, temp_e))

            R_gf_g.append(calc_R_new(m_g, temp_g, temp_f))
            R_gf_e.append(calc_R_new(m_e, temp_g, temp_f))
            R_gf_f.append(calc_R_new(m_f, temp_g, temp_f))
...
if d3_plot:
    ...
    #m_g_ideal = template_analytic(ts, wr, w_rot_r, kappa, chi_0, u_g)
    #m_e_ideal = template_analytic(ts, wr, w_rot_r, kappa, chi_1, u_e)
    #m_f_ideal = template_analytic(ts, wr, w_rot_r, kappa, chi_2, u_f)
    m_g_ideal = temp_g
    m_e_ideal = temp_e
    m_f_ideal = temp_f

    #add ideal R calculated from analytic solution of template
    R_eg_g.append(calc_R_new(m_g_ideal, temp_e, temp_g))
    R_eg_e.append(calc_R_new(m_e_ideal, temp_e, temp_g))
    R_eg_f.append(calc_R_new(m_f_ideal, temp_e, temp_g))

    R_fe_g.append(calc_R_new(m_g_ideal, temp_f, temp_e))
    R_fe_e.append(calc_R_new(m_e_ideal, temp_f, temp_e))
    R_fe_f.append(calc_R_new(m_f_ideal, temp_f, temp_e))

    R_gf_g.append(calc_R_new(m_g_ideal, temp_g, temp_f))
    R_gf_e.append(calc_R_new(m_e_ideal, temp_g, temp_f))
    R_gf_f.append(calc_R_new(m_f_ideal, temp_g, temp_f))
...
if d2_plot and d3_plot:
    ...
    plt.plot(R_g_1s[-1], R_g_2s[-1], 'o', label='g analy')
    plt.plot(R_e_1s[-1], R_e_2s[-1], 'o', label='e analy')
    plt.plot(R_f_1s[-1], R_f_2s[-1], 'o', label='f analy')
    plt.xlabel(r'$R_1$')
    plt.ylabel(r'$R_2$')
    plt.legend()
    plt.show()
...
d2_plot_parameter_variation = True

if d2_plot_parameter_variation:
    for kappa in np.linspace(1 / 10 * chi_0, 200 * chi_0, 100):
        template_analytic(ts, wr, w_rot_r, kappa, chi_0, u_g)